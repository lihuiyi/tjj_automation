...
public class XS {
	...
	public XS(String ChromePath, String ChromeDriverPath, int timeout) {
	...
	public static void main(String[] args) {
		...
		XS xs = new XS(ChromePath, ChromeDriverPath, timeout);
		xs.login(url, userName, passWord);
		xs.run();
	}
	...
	public void run_table(WebDriver driver, Actions actions, int timeout, int j){
		...
		// 主要业务活动
		xpath = "//*[@id='businessData']/tbody/tr[19]/td[2]/div";
		wait.element_presence(driver, timeout, xpath);
		WebElement zyywhd_div = driver.findElement(By.xpath(xpath));
		String text = zyywhd_div.getAttribute("innerText");
		actions.doubleClick(zyywhd_div).build().perform(); // 鼠标双击指定的元素
		
		xpath = "//*[@id='businessData']/tbody/tr[19]/td[2]/input";
		WebElement zyywhd_input = driver.findElement(By.xpath(xpath));
		actions.doubleClick(zyywhd_input).build().perform(); // 鼠标双击指定的元素
		zyywhd_input.clear();
		
		// 销售 （石、种子、树、林、苗、饲料）
		String new_text = text.replace("销售", "零售");
		zyywhd_input.sendKeys(new_text);
